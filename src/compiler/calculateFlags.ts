...
export function calculateFlags(newValue) {
    ...
    if (newValue == 0) {  // if value is zero, then set zero flag to 1
        registers.set('zf', 1)
    }
    if (newValue < 0) { // if most significant bit is set to one, then set sign flag to 1
        registers.set('sf', 1)
    }

    let newValueInBinary: string = intToFormattedString(newValue, 'bin', 16)
